...
let articulosDeVarios = [
    {id:46,nombre:'Garrafa',detalle:'G.L.P.',precio:1,img:'./../../img/Productos/Varios/1.jpg'}
]
...
function render(data) {
  ...
  articulosDeVarios.forEach((art, i) => {
    const estadoDelArticulo = data.estadoDelProdVarios[i]
   ...
   targeta.innerHTML = `
      ...
      <div class="contenido">
          ...
          ${localStorage.getItem("admin") === "true"
            ? `<p class="precioClas" contenteditable="true" onblur="editarPrecioVarios(${i}, this.innerText)"><b>Bs.</b> ${art.precio}</p>`
...
function cargarTodo(){
    Promise.all([
      fetch('/disponibleAgotadoVarios').then(res=>res.json()),//Apunta al archivo .JSON (estado Disponible Agotado)
      fetch('/precioDeLosVarios').then(res=>res.json())//Apunta al archivo .JSON (precioDeLosArticulos.JSON)
    ]).then(([estadoDisponibleAgotado1, actualizarPrecio2])=>{//Son simplemente parametros locales
      articulosDeVarios = articulosDeVarios.map((art, i)=>({//Es el array de objetos de los articulos de este archivo
        ...art,
...
socket.on('estadoActualizadoVarios', (data) => {
  render(data)
})//Cambiar de cargarEstado() a cargarTodo() y encima de cargarTodo() el precio editable...

socket.on('PrecioActualVarios', (actualizarPrecioParam)=>{//PrecioActual viene del server.js io.emit
  articulosDeVarios = articulosDeVarios.map((art, i)=>({//Es el array de objetos de los articulos de este archivo
    ...art,
    precio:actualizarPrecioParam[i]//El precio viene del Array de Objetos articulos de este archivo
  }))
  fetch('/disponibleAgotadoVarios')//Viene desde el archivo .json
    .then(res => res.json())
...
function editarPrecioVarios(index, texto) {//Esta funcion enviamos al <p contenteditable onblur=() >
  const arrayJsonPrecVarios = parseFloat(texto.replace(/[^0-9.]/g, ''))// El contenido del archivo precio.JSON
  if (isNaN(arrayJsonPrecVarios)) return
  fetch('/precioDeLosVarios', {// es el nombre del archivo precio.JSON
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ index, arrayJsonPrecVarios })
  })
}