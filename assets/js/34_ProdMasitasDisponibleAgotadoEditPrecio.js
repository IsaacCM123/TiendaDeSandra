...
let articuloDeMasitas = [
    {id:69,nombre:'Queque',detalle:'Casero',precio:1,img:'./../../img/Productos/Masitas/1.jpg'}
]
...
function render(data) {
  ...
  articuloDeMasitas.forEach((art, i) => {
    const estadoDelArticulo = data.estadoDelProdMasita[i]
   ...
   targeta.innerHTML = `
      ...
      <div class="contenido">
          ...
          ${localStorage.getItem("admin") === "true"
            ? `<p class="precioClas" contenteditable="true" onblur="editarPrecioMasitas(${i}, this.innerText)"><b>Bs.</b> ${art.precio}</p>`
...
function cargarTodo(){
    Promise.all([
      fetch('/disponibleAgotadoMasitas').then(res=>res.json()),//Apunta al archivo .JSON (estado Disponible Agotado)
      fetch('/precioDeLasMasitas').then(res=>res.json())//Apunta al archivo .JSON (precioDeLosArticulos.JSON)
    ]).then(([estadoDisponibleAgotado1, actualizarPrecio2])=>{//Son simplemente parametros locales
      articuloDeMasitas = articuloDeMasitas.map((art, i)=>({//Es el array de objetos de los articulos de este archivo
        ...art,
...
function cambiarEstado(index, value) {
  fetch('/disponibleAgotadoMasitas', {
    method: 'POST',
...
socket.on('estadoActualizadoMasita', (data) => {
  render(data)
})//Cambiar de cargarEstado() a cargarTodo() y encima de cargarTodo() el precio editable...

socket.on('PrecioActualMasita', (actualizarPrecioParam)=>{//PrecioActual viene del server.js io.emit
  articuloDeMasitas = articuloDeMasitas.map((art, i)=>({//Es el array de objetos de los articulos de este archivo
    ...art,
    precio:actualizarPrecioParam[i]//El precio viene del Array de Objetos articulos de este archivo
  }))
  fetch('/disponibleAgotadoMasitas')//Viene desde el archivo disponibleAgotado.json
    .then(res => res.json())
...
function editarPrecioMasitas(index, texto) {//Esta funcion enviamos al <p contenteditable onblur=() >
  const arrayJsonPrecMasita = parseFloat(texto.replace(/[^0-9.]/g, ''))// El contenido del archivo precio.JSON
  if (isNaN(arrayJsonPrecMasita)) return
  fetch('/precioDeLasMasitas', {// es el nombre del archivo precio.JSON
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ index, arrayJsonPrecMasita })
  })
}